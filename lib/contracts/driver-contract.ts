 ...
 * signature. Returns null if the driver isn't approved yet.
 */
...
export async function ensureDriverContract(admin: any, userId: string): Promise<DriverContractRow | null> {
  ...
  const { data: profile } = await A.from('driver_profiles')
    .select('id, verification_tier, verification_status, onboarding_state, next_of_kin_name, next_of_kin_phone, next_of_kin_relationship')
    .eq('user_id', userId)
    .single();
  ...
  if (!contract) {
    ...
    const reference = `DRV-${String(profile.id).slice(0, 8)}-${Date.now()}`;

    const terms = buildDriverContractTerms({
      reference,
